...
package provide sdrblk::block-sequence 1.0.0
...
snit::type sdrblk::block-sequence {
    ...
    constructor {args} {
	if {$verbose(construct)} { puts "block-sequence $self constructor $args" }
	install core using ::sdrblk::block-core %AUTO% -coreof $self -type sequence {*}$args

	sdrblk::comp-stub ::sdrblk::[$core cget -name]
	foreach element [$core cget -sequence] {
	    package require $element
	    lappend sequence [$element %AUTO% -partof $core]
	}

	foreach \
	    last [concat [list {}] [lrange $sequence 0 end-1]] \
	    element $sequence \
	    next [concat [lrange $sequence 1 end] [list {}]] \
	    {
		if {$last ne {} && $next ne {}} {
