...
int sequencer_input(ClientData instanceData, char *buf, int bufSize, int *errorCodePtr) {
  sequencer_instance_t *sqi = (sequencer_instance_t *)instanceData;
  int err = snd_sequencer_read(sqi->sequencer, buf, bufSize);
  if (err < 0) {
    *errorCodePtr = -err;
    return -1;
  } else {
...
int sequencer_output(ClientData instanceData, const char *buf, int toWrite, int *errorCodePtr) {
  sequencer_instance_t *sqi = (sequencer_instance_t *)instanceData;
  int err = snd_sequencer_write(sqi->sequencer, buf, toWrite);
  if (err < 0) {
    *errorCodePtr = -err;
    return -1;
  } else {
...
int sequencer_get_handle(ClientData instanceData, int direction, ClientData *handlePtr) {
  sequencer_instance_t *sqi = (sequencer_instance_t *)instanceData;
...
int sequencer_block(ClientData instanceData, int mode) {
  sequencer_instance_t *sqi = (sequencer_instance_t *)instanceData;
  return snd_sequencer_nonblock(sqi->sequencer, mode == TCL_MODE_NONBLOCKING);
}

static Tcl_ChannelType sequencer_channel_type = {
  "sequencer",			/* typename */
  TCL_CHANNEL_VERSION_5,	/* version */
  sequencer_close,		/* closeProc */
  sequencer_input,		/* inputProc */
  sequencer_output,		/* outputProc */
  NULL,				/* seekProc */
  NULL,				/* setOptionProc */
  NULL,				/* getOptionProc */
  sequencer_watch,		/* watchProc */
  sequencer_get_handle,		/* getHandleProc */
  NULL,				/* close2Proc */
  NULL,				/* blockModeProc */
  NULL,				/* flushProc */
  NULL,				/* handlerProc */
...
*/
static int alsa_sequencer(ClientData clientData, Tcl_Interp *interp, int argc, Tcl_Obj* const *objv) {
  if (argc > 1) {
...
int DLLEXPORT Alsa_sequencer_Init(Tcl_Interp *interp) {
...
#endif
  Tcl_PkgProvide(interp, "alsa::sequencer", "0.0.1");
  Tcl_CreateObjCommand(interp, "alsa::sequencer", alsa_sequencer, NULL, NULL);
  return TCL_OK;
}
