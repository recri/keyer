...
#include "../dspmath/filter_complex_goertzel.h"
#include "../dspmath/midi.h"
#include "framework.h"

typedef struct {
#include "framework_options_vars.h"
  filter_complex_goertzel_options_t fg;
  float on_threshold;
  float off_threshold;
} options_t;
...
static void _update(_t *dp) {
  if (dp->modified) {
    ...
    filter_complex_goertzel_configure(&dp->fg, &dp->opts.fg);
  }
}
...
static void *_init(void *arg) {
  ...
  void *p = filter_complex_goertzel_preconfigure(&dp->fg, &dp->opts.fg); if (p != &dp->fg) return p;
  filter_complex_goertzel_configure(&dp->fg, &dp->opts.fg);
  dp->on = 0;
  return arg;
}
...
static int _process(jack_nframes_t nframes, void *arg) {
  ...
  float *in0 = jack_port_get_buffer(framework_input(dp,0), nframes);
  float *in1 = jack_port_get_buffer(framework_input(dp,1), nframes);
  ...
  for (int i = 0; i < nframes; i++) {
    if (filter_complex_goertzel_process(&dp->fg, *in0++ + *in1++ * I)) {
      dp->power = dp->fg.power;
      dp->energy = dp->fg.energy;
      dp->frame = sdrkit_last_frame_time(arg)+i;
      cmd = 0;
      float mag = cabsf(dp->power);
      if (dp->on != 0 && mag < dp->opts.off_threshold) {
	cmd = MIDI_NOTE_OFF;	/* note change off */
      } else if (dp->on == 0 && mag > dp->opts.on_threshold) {
...
//   the scaled real value of the filter result,
//   the scaled imag value of the filter result,
...
static int _command(ClientData clientData, Tcl_Interp *interp, int argc, Tcl_Obj* const *objv) {
  ...
  if (data->modified) {
    void *e = filter_complex_goertzel_preconfigure(&data->fg, &data->opts.fg); if (e != &data->fg) {
      data->opts = save;
...
#include "framework_options.h"
  { "-freq",      "frequency", "AFHertz", "700.0", fw_option_float, fw_flag_none, offsetof(_t, opts.fg.hertz),     "frequency to tune in Hz, abs < samplerate/2"  },
  { "-bandwidth", "bandwidth", "BWHertz", "750.0", fw_option_float, fw_flag_none, offsetof(_t, opts.fg.bandwidth), "bandwidth of output signal in Hz, abs < samplerate/2" },
...
static const framework_t _template = {
  ...
  2, 0, 0, 1, 0,		// inputs,outputs,midi_inputs,midi_outputs,midi_buffers
...
int DLLEXPORT Filter_complex_goertzel_Init(Tcl_Interp *interp) {
  return framework_init(interp, "sdrtcl::filter-complex-goertzel", "1.0.0", "sdrtcl::filter-complex-goertzel", _factory);
}
