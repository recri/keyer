...
/*
** A simplification of the various taps specialized
** to simply grab a buffer of samples and deactivate.
**
** Hmm, complications.  Activation means that the process
** callback gets called, but the samples won't be valid
** until the ports get connected.  So, to start a snap
** means to activate and connect and then start collecting
** samples.  Too complicated to think through just yet.
**
** In the end, I think, you reuse samples snapped for any
** reason if they're useable.  Which means using the 
** samples collected for the panadapter.
**
** In the final end, you use the samples buffered for the
** overlap save filter, since they're always available.
*/
