* Ascii/Iambic morse keyer using Jack audio connection kit
  ...
  plugins and a variety of Tcl based tools for playing with them.

  The [[keyer-control]] panel launches iambic and ascii keyers with 
  independent tone generators, wires them up, and allows their
  parameters to be fiddled with.

  The [[pileup]] application resurrects an application written by Richard
  Everitt, G4ZFE, for testing your ability to read callsigns in a CW
  pileup.

  The intended usage of these modules is as Tcl extensions, but I've
  left the following modules packaged as stand alone commands, though
  they are simply Tcl scripts.

  The [[keyer-tone]] plugin generates i/q cw tones from midi events.

  The [[keyer-ascii]] plugin translates ascii text into midi events to
  drive the tone generator.

  The [[keyer-iambic]] plugin translates midi events from an iambic paddle
  into midi events to drive the tone generator.

  The [[keyer-decode]] plugin translates midi events into dits, dahs, and
...
* Paddle Interface
  The whole point was to get CW keying information into the computer with
...
** common options for morse code timing
  ...
  between the words.
** keyer-ascii - ascii to morse keyer
...
** keyer-iambic - iambic paddle to morse keyer
