...
* Usage
  ...
  In particular, bin/keyer-control, bin/keyer-scope, and bin/pileup
  are written to automatically hook themselves up in a useful way, and
  they are the applications which I test to make sure they work before
...
** pileup - morse code training
   ...
   original pileup program or they are words with up to N dit clocks
   from the start of the first element to the end of the last
   element.
