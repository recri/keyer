...
** TODO - MidiKey: to debounce with a period in microseconds.
   Mangled the existing Bounce class, still not quite right.
   Went back and implented a millisecond/20 step filter debouncer,
   still getting some clicks.
** TODO - ptt_delay - a plugin to take keyer output, send a ptt signal
   immediately and pass through the keyer signal after a delay to let
   the system activate.
** TODO - ptt_mute - a gain plugin with a ptt input that mutes the
   audio channel.
