...
/*
** This is the filterbank code from dttsp.
** How exactly it works would depend on knowing the filter passed in
*/
#if 0
/** 
* @file filterbank.h
...
extern FIRDownsampler newFIRDownsampler(RealFIR filter,unsigned int downsample, unsigned int channel, COMPLEX *in, COMPLEX *out);
extern FIRUpsampler newFIRUpsampler(RealFIR filter,unsigned int upsample, unsigned int channel, COMPLEX *in, COMPLEX *out);
extern FIRPFB newFIRPFBChannelizer(RealFIR filter,unsigned int numchans, COMPLEX *in, COMPLEX *out, int pbits);
extern FIRPFB newFIRHBPFBChannelizer(RealFIR filter, COMPLEX *sigin, COMPLEX *sigout);
...
FIRDownsampler newFIRPDownsampler(RealFIR filter, unsigned int downsample, unsigned int channel, COMPLEX *sigin, COMPLEX *sigout) {
  FIRDownsampler tmp;
  unsigned int i,j;
  ...
  tmp->filter_partition = (REAL    **)safealloc(downsample,sizeof(REAL    *),"fb partition pointers");
  tmp->filter_delayline = (COMPLEX **)safealloc(downsample,sizeof(COMPLEX *),"fb partition pointers");
  for (i=0;i<tmp->downsample;i++) { 
    tmp->filter_partition[i] = (REAL *)safealloc(tmp->depth,sizeof(REAL),"fb partition delay line");
    tmp->filter_delayline[i] = (COMPLEX *)safealloc(tmp->depth,sizeof(COMPLEX),"fb partition delay line");
  }
  ...
  for (i=0;i<tmp->downsample;i++)
    tmp->rotator[i] = Cmplx((REAL)cos(i*channel*2.0*M_PI/tmp->downsample),(IMAG)sin(i*channel*2.0*M_PI/tmp->downsample));

  return tmp;
}

FIRUpsampler newFIRPUpsampler(RealFIR filter, unsigned int upsample, unsigned int channel, COMPLEX *sigin, COMPLEX *sigout) {
  FIRUpsampler tmp;
  unsigned int i,j;
  ...
  // Set up numchans partitions
  tmp->filter_partition = (REAL    **)safealloc(upsample,sizeof(REAL    *),"fb partition pointers");
  tmp->filter_delayline = (COMPLEX **)safealloc(upsample,sizeof(COMPLEX *),"fb partition pointers");
  ...
  for (i=0;i<tmp->upsample;i++)
    tmp->rotator[i] = Cmplx((REAL)cos(i*channel*2.0*M_PI/tmp->upsample),(IMAG)sin(i*channel*2.0*M_PI/tmp->upsample));

  return tmp;
}

FIRPFB newFIRHBPFBChannelizer(RealFIR filter, COMPLEX *sigin, COMPLEX *sigout) {
  FIRPFB tmp;
  unsigned int i,j;
  ...
  // Set up numchans partitions
  tmp->filter_partition = (REAL    **)safealloc(2,sizeof(REAL    *),"fb partition pointers");
  tmp->filter_delayline = (COMPLEX **)safealloc(2,sizeof(COMPLEX *),"fb partition pointers");
...
FIRPFB newFIRPFBChannelizer(RealFIR filter, unsigned int numchans, COMPLEX *sigin, COMPLEX *sigout, int pbits) {
  FIRPFB tmp;
  unsigned int i,j;
  ...
  // Set up numchans partitions
  tmp->filter_partition = (REAL    **)safealloc(numchans,sizeof(REAL    *),"fb partition pointers");
  tmp->filter_delayline = (COMPLEX **)safealloc(numchans,sizeof(COMPLEX *),"fb partition pointers");
  for (i=0;i<tmp->numchans;i++) { 
    tmp->filter_partition[i] = (REAL *)safealloc(tmp->depth,sizeof(REAL),"fb partition delay line");
    tmp->filter_delayline[i] = (COMPLEX *)safealloc(tmp->depth,sizeof(COMPLEX),"fb partition delay line");
  }
  ...
  tmp->pchan = fftwf_plan_dft_1d(tmp->numchans, (fftwf_complex *)tmp->fftbuffer, (fftwf_complex *)tmp->sigout, FFTW_FORWARD, pbits);
